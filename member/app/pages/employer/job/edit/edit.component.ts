...
export class EmployerJobEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.comForm = new FormGroup({
      ...
      serviceage: new FormControl('', [Validators.required]),
    ...
    this.jobSvc.getJob(this.user.key, this.id).then(res => {
      ...
      for (const key in this.comForm.value) {
        if (res.result[key]) {
          this.comForm.get(key).setValue(res.result[key]);
        }
      }
    });
  ...
  showServices() {
    this.pickerSvc.showCity(SERVICES_DATA, this.comForm.get('services').value).subscribe(res => {
  ...
  save() {
    ...
    this.jobSvc.post(this.comForm.value).then(res => {
      ...
      if (res.code === '0000') {
        this.overlaySvc.hide();
        this.dialogSvc.show({content: '您发布的职位保存成功', cancel: '', confirm: '我知道了'}).subscribe();
      }
    });
  }
