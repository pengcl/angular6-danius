...
@Component({
  selector: 'app-employee-intention-edit',
  templateUrl: './edit.component.html',
  styleUrls: ['./edit.component.scss']
})
export class EmployeeIntentionEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.intentionsForm.get('id').setValue(this.id);
    ...
    this.userSvc.getIntent(this.user.key, this.id).then(res => {
      this.intent = res.result;
      this.intentionsForm.get('positionid').setValue(res.result.positionid);
      this.intentionsForm.get('positionname').setValue(res.result.intentname);
      this.intentionsForm.get('tradeid').setValue(res.result.tradeid);
      this.intentionsForm.get('tradename').setValue(res.result.tradename);
      this.intentionsForm.get('intentprovince').setValue(res.result.intentprovince);
      this.intentionsForm.get('intentprovincecode').setValue(res.result.intentprovincecode);
      this.intentionsForm.get('intentcity').setValue(res.result.intentcity);
  ...
  remove() {
    this.toastSvc.loading('删除', 9999);
    this.userSvc.delIntent(this.user.key, this.id).then(res => {
      this.toastSvc.hide();
      if (res.code === '0000') {
        this.location.back();
      }
